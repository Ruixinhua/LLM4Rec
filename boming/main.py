...
from common import function_dispatcher, template_dispatcher
from prompt import *
        ...
        for filename in os.listdir(folder_path):
            ...
            if filename.endswith(".csv"):
                ...
                for index in range(origin_df.shape[0]):
                    ...
                    print("--------------------------------------------------")
                    prompt_input = template.substitute(history=line['history'], candidate=line["candidate"])
...
if __name__ == '__main__':
    ...
    for tag in ['init', 'final', '5074']:
        template = template_dispatcher.get(tag)
        run(template, tag=tag)
